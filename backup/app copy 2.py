...
# ⚠ 기본값을 8ch(hybrid_black)로 두는 것을 권장합니다. (데이터셋과 가장 일반적으로 맞음)
...
def get_pick_channels(device_name: str):
    return DEVICE_TO_PICK_CHANNELS.get(device_name, DEVICE_TO_PICK_CHANNELS['hybrid_black'])
...
def create_app():
    ...
    # 모델 채널 수와 서버 디바이스 채널 수 점검
    expected_ch = DEVICE_TO_CHANNELS.get(EEG_DEVICE)
    cfg_ch = int(CFG.get("n_chans", expected_ch))
    if expected_ch != cfg_ch:
        app.logger.warning(f"⚠ 모델 n_chans({cfg_ch})과 서버 디바이스({EEG_DEVICE}:{expected_ch})가 불일치할 수 있습니다. HF_REPO_ID 확인 필요.")
            ...
            if req_device != EEG_DEVICE:
                return jsonify({
                    ...
                    "message": f"Server loaded for '{EEG_DEVICE}'. You requested '{req_device}'. "
                               f"Restart with EEG_DEVICE={req_device} (and matching HF_REPO_ID) to switch."
                }), 400

            # 이 요청에서 사용할 pick_channels를 강제 주입
