...
def create_app():
    ...
    def infer():
        ...
        요청(JSON):
        {
          "file_path": "dataset/sub-044/eeg/sub-044_task-eyesclosed_eeg.set",
          "true_label": "CN",                 # (옵션) "CN"/"AD"/"FTD" or "C"/"A"/"F" or 0/1/2
          "subject_id": "sub-044",            # (옵션, 없으면 경로에서 자동 추출)
          "callback_url": "http://spring/...",# (옵션) 제공 시 비동기 처리(그 URL로도 POST)
          "enforce_two_minutes": true         # (옵션, 기본 true: 2분 미만이면 에러)
        }
        응답(JSON):
        {
          "status": "ok",
        ...
        try:
            ...
            subject_id   = payload.get("subject_id") or extract_subject_id(file_path)
            ...
            # ✅ 결과를 Spring에도 자동 POST (prob_mean 포함 전체 result)
            if SPRING_AUTO_POST:
...
def _run_infer(file_path: str, subject_id: str, true_label, enforce_2min: bool):
    ...
    # Spring에 필요한 핵심 필드만 추림 (+ subject 정확도/예측 라벨 포함)
    res = {
        ...
        "segment_accuracy": out["segment_accuracy"],            # true_label 없으면 null

        # subject 기준
        "subject_pred_index": out.get("subject_pred_index"),
        "subject_pred_label": out.get("subject_pred_label"),
        "subject_accuracy": out.get("subject_accuracy"),        # true_label 있으면 0/1, 없으면 null

        # ✅ prob_mean 포함 (그대로 Spring 전송)
        "prob_mean": out["prob_mean"],

        # 부가정보
        "window": out["window"],                                # {"start": s_best, "need": need}
